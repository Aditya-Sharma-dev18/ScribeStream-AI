...
def stream_workflow(
    ...
    # ✅ FIX #1: sections must be a dict (matches backend State reducer)
    workflow_input = {
        "topic": topic,
        "sections": {},
    }
        ...
        for raw_chunk in stream:
            ...
            for node_name, raw_node_update in updates.items():
                ...
                if node_name == "router":
                    ...
                    # ✅ FIX #2: backend uses "need_research" (singular)
                    needs_research = bool(
                        node_update.get(
                            "need_research",
                            False,
                ...
                # =================================================
                # Diagram orchestrator (backend's image subgraph)
                # =================================================
                elif node_name == "diagram_orchestrator":
                    images = node_update.get("images", {})
                    ...
                    yield create_sse_event(
                        {
                            "type": "stage",
                            "id": "diagram",
                            "label": "Generate technical visuals",
                            "status": "completed",
                            "detail": (
                                f"Prepared {len(images)} "
                                "section figure"
                ...
                # =================================================
                    ...
                    sections_update = node_update.get(
                        "sections",
                        {},
                    )

                    # ✅ FIX #3: backend returns sections as a dict
                    # {task_id: markdown}, not a list of tuples.
                    if isinstance(sections_update, dict):
                        items = list(sections_update.items())
                    elif isinstance(sections_update, list):
                        items = sections_update
                    else:
                        items = []
