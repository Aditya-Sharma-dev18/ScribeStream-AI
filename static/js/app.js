...
function showPlan(plan) {
    ...
    // Match backend Plan schema:
    // { title, target_audience, technical_depth, tasks }
    planMeta.innerHTML = `
        <div>
            <span>Audience</span>
            <strong>
    ...
    tasks.forEach((task, index) => {
        ...
        // Match backend Task schema:
        // { id, title, brief, target_word_count,
        //   key_takeaways, section_role,
        //   include_code_or_math }
        const tags = [];

        if (task.include_code_or_math) {
            tags.push("Code/Math");
        }

        if (task.section_role) {
            tags.push(
                String(task.section_role)
                    .replaceAll("_", " "),
            );
        }
